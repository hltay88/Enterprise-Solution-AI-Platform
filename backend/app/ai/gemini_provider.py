...
class GeminiProvider(AIProvider):
    ...
    async def generate_proposal_content(
        ...
        *,
        ...
        system_prompt = load_prompt("proposal_generate.txt")
        user_prompt = (
        ...
        response = await self._generate(
            action="generate proposal content",
            system_prompt=system_prompt,
            user_prompt=user_prompt,
            temperature=0.2,
        )
        payload = parse_json_object(
            response.text or "",
            empty_message="AI provider returned an empty proposal response",
            invalid_message="AI provider returned invalid JSON for proposal content",
        )
