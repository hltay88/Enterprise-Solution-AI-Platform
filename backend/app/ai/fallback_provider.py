    ...
    async def recommend_architecture(
        ...
        *,
        ...
        for index, provider in enumerate(self.providers):
            name = provider.__class__.__name__
            try:
                result = await provider.recommend_architecture(
                    published_rkm,
                ...
                if index > 0 and isinstance(provider, LocalAIProvider):
                    ...
                    logger.warning("Using fallback provider %s for architecture", name)
                return payload
            except AppError as exc:
                is_last = index == len(self.providers) - 1
                if exc.code not in _FALLBACK_CODES or is_last:
                ...
                errors.append(f"{name}: {exc.message}")
                logger.warning(
                    "%s architecture unavailable (%s); trying next",
                    name,
                    exc.code,
                )
    ...
    async def recommend_architectures(
        ...
        *,
        ...
        for index, provider in enumerate(self.providers):
            name = provider.__class__.__name__
            try:
                result = await provider.recommend_architectures(
                    published_rkm,
                ...
                if index > 0 and isinstance(provider, LocalAIProvider):
                    ...
                    logger.warning(
                        "Using fallback provider %s for architecture candidates",
                        name,
                    )
                return payload
            except AppError as exc:
                ...
                errors.append(f"{name}: {exc.message}")
                logger.warning(
                    "%s architecture candidates unavailable (%s); trying next",
                    name,
                    exc.code,
                )
    ...
    async def generate_proposal_content(
        ...
        *,
        ...
        for index, provider in enumerate(self.providers):
            name = provider.__class__.__name__
            try:
                result = await provider.generate_proposal_content(
                    snapshot,
                    content_plan,
                ...
                errors.append(f"{name}: {exc.message}")
                logger.warning(
                    "%s proposal generation unavailable (%s); trying next",
                    name,
                    exc.code,
                )
    ...
    async def generate_sow_content(
        ...
        *,
        ...
        for index, provider in enumerate(self.providers):
            name = provider.__class__.__name__
            try:
                result = await provider.generate_sow_content(
                    snapshot,
                    content_plan,
                    prompt_version=prompt_version,
                )
                ...
                errors.append(f"{name}: {exc.message}")
                logger.warning(
                    "%s SOW generation unavailable (%s); trying next",
                    name,
                    exc.code,
                )
    ...
    async def generate_solution_design_content(
        ...
        *,
        ...
        for index, provider in enumerate(self.providers):
            name = provider.__class__.__name__
            try:
                result = await provider.generate_solution_design_content(
                    snapshot,
                    content_plan,
                    prompt_version=prompt_version,
                )
                ...
                errors.append(f"{name}: {exc.message}")
                logger.warning(
                    "%s solution design generation unavailable (%s); trying next",
                    name,
                    exc.code,
                )
