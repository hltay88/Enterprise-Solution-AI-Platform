"""SOW generation orchestration (Sprint 4.3)."""
...
from app.services.sow_content_planner import SowContentPlanner
...
class SowGenerationService:
    def __init__(self, db: Session) -> None:
        ...
        self.planner = SowContentPlanner()
    ...
    async def generate(
        ...
        body = body or DeliverableGenerateIn(document_type="sow")
        if body.document_type != "sow":
            raise ValidationAppError("SowGenerationService expects document_type=sow")
        ...
        template, template_version = self.templates.resolve_sow_template()
        plan = self.planner.build(
            snapshot.payload_json or {},
            list(template_version.sections_json or []),
        )

        prompt_version = "sow_v1"
        provider = get_ai_provider()
        raw = await provider.generate_sow_content(
            snapshot.payload_json or {},
            plan,
            prompt_version=prompt_version,
        )
        try:
            payload = SowContentPayload.model_validate(raw)
            ...
            raise ValidationAppError(
                f"SOW AI output failed schema validation: {exc}"
            ) from exc
        ...
        AuditService(self.db).record(
            ...
            summary=f"Generated SOW draft '{document.title}'",
